...
burial_data <- burial_raw %>%
  ...
  # Rename columns for clarity (only columns that need new names)
  rename(
...
burial_data %>%
  dplyr::filter(!is.na(burial_description) & burial_description != "-") %>%
...
burial_data %>%
  dplyr::filter(!is.na(goods_chamber) & goods_chamber != "-") %>%
...
drinking_vessel_graves <- burial_data %>%
  ...
  dplyr::filter(has_drinking_vessel)
